...
const SignIn = () => {
  ...
  const onSubmit = async (data: SignInFormValues) => {
    ...
    setIsLoading(true);
    setServerError(null); // Limpa qualquer erro de servidor anterior
    clearErrors(); // Limpa erros de validação anteriores, se houver
  ...
  return (
    <section className="container mx-auto lg:px-[8%] min-h-[calc(100vh-128px)] grid grid-cols-1 lg:grid-cols-2 gap-8 lg:mt-6">
      <div className="flex flex-col place-self-center gap-y-8 mx-auto w-full min-w-2xs max-w-sm p-8 lg:p-4">
        <h2 className="text-2xl tracking-tight font-bold">{t("title")}</h2>
        <p className="text-zinc-500 text-sm font-medium tracking-tight">
          {t("notHaveAccount")}{" "}
          <Link href="/auth/sign-up" className="text-indigo-600 font-semibold">
            {t("createAccountNow")}
          </Link>
        </p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-8">
          ...
          <div className="flex justify-between items-center text-sm">
            ...
            <p className="font-semibold text-indigo-600 cursor-pointer">
              {t("forgotPassword")}
            </p>
          </div>
      ...
      <div className="relative hidden lg:block ">
        <Image
          src="/pexels-lucas-brown-368306455-16135761.jpg"
          alt="woman posing with a view of the sky in the background / mulher posando com vista para o céu ao fundo"
          fill
